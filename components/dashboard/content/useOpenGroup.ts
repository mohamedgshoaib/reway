...
export function useOpenGroup({ bookmarks, deferredSearchQuery }: UseOpenGroupOptions) {
  const pendingRequestsRef = useRef(
    new Map<string, (response: { ok: boolean; count?: number; error?: string } | null) => void>(),
  )
  ...
  const handleOpenGroup = useCallback(
    async (groupId: string) => {
      ...
      const targetBookmarks = bookmarks.filter((bookmark) => {
        const matchesGroup = isAllBookmarksGroupId(groupId)
          ? true
      ...
      const response = await new Promise<{
        ...
        window.postMessage({ type: "reway_open_group", requestId, groupId, urls }, "*")
      })

      if (response?.ok) {
        toast.success(`Opened ${response.count ?? targetBookmarks.length} tabs via extension`)
        return
      }
