...
export function useFolderDnd<T extends { id: string; group_id?: string | null }>(options: {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const activeGroupId = active.data?.current?.sortable?.containerId as string | undefined
    const overGroupId = over?.data?.current?.sortable?.containerId as string | undefined
