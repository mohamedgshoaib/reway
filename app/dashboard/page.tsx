...
export default async function DashboardPage() {
  ...
  const rowContent = parseRowContent(cookieStore.get("reway.dashboard.rowContent")?.value)
  const commandMode = parseCommandMode(cookieStore.get("reway.dashboard.commandMode")?.value)
  const showNotesTodos = parseShowNotesTodos(
    cookieStore.get("reway.dashboard.showNotesTodos")?.value,
  )

  const paletteTheme = parsePaletteTheme(cookieStore.get("reway.dashboard.paletteTheme")?.value)

  const layoutDensity = parseLayoutDensity(cookieStore.get("reway.dashboard.layoutDensity")?.value)
